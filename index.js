...
const mongoURI = 'mongodb+srv://Haileopia:0939100302@cluster0.zanpy7q.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0'; // MongoDB Atlas connection string
...
MongoClient.connect(
  mongoURI,
  { useNewUrlParser: true, useUnifiedTopology: true, tls: true }, // Enable TLS/SSL
  (err, client) => {
    if (err) {
    ...
    app.get('/start', (req, res) => {
      res.send('Welcome to FoodieBot, your personal food ordering assistant! What would you like to order today?');
    });
    ...
    app.post('/order', express.json(), (req, res) => {
      ...
      db.collection('orders').insertOne({ chatId, item: menuItem.name, price: menuItem.price }, (err, result) => {
        if (err) {
          console.error('Error inserting order into MongoDB:', err);
          res.status(500).send('Sorry, there was an error processing your order. Please try again later.');
          return;
        }
        res.send(`You've ordered ${menuItem.name} for $${menuItem.price}. Please confirm your order by typing /confirm.`);
      });
    });
    ...
    app.post('/confirm', express.json(), (req, res) => {
      ...
      db.collection('orders').findOne({ chatId }, (err, order) => {
        ...
        res.send(`Your order for ${order.item} has been confirmed! It will be prepared and delivered shortly.`);
